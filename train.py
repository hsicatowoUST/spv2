...
def train(epoch, model, optimizer, scheduler, scaler, train_loader, cfg, logger, writer):
    ...
    for name, param in model.named_parameters():
        assert param.is_cuda, f"Parameter {name} is not on CUDA"
    #debug
    ...
    for i, batch in enumerate(train_loader, start=1):
        ...
        #debug
        batch = [v.cuda() if hasattr(v, 'cuda') and not v.is_cuda else v for v in batch]
        for k, v in enumerate(batch):
            if hasattr(v, 'is_cuda'):
                assert v.is_cuda, f"Batch tensor {k} is not on CUDA"
        #debug
...
def validate(epoch, model, optimizer, val_loader, cfg, logger, writer):
    ...
    world_size = dist.get_world_size() if cfg.dist else 1
    sem_point_eval = PointWiseEval(num_classes=cfg.model.semantic_classes,ignore_label=35,gpu_num=world_size)
    instance_eval = InstanceEval(num_classes=cfg.model.semantic_classes,ignore_label=35,gpu_num=world_size)
    meter_dict = {}
    ...
    logger.info("Evaluate semanftic segmentation")
...
def main():
    ...
    val_loader = build_dataloader(args,val_set, training=False, dist=False, **cfg.dataloader.test)
    ...
    for group in optimizer.param_groups:
        for p in group['params']:
            assert p.is_cuda, "Optimizer parameter is not on CUDA"
    # debug
    # scheduler
