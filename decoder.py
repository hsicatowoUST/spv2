...
class Decoder(nn.Module):
    ...
    def query_for_dn(self, stage_list, queries, query_pos):
        ...
        # Handle empty query_feats case
        if len(query_feats) == 0:
            print("INFO: len(query_feats) == 0 - No valid instances for denoising in query_for_dn")
            # No valid instances for denoising, return original queries without DN
            tgt_mask = None
            dn_args = torch.tensor([]).to("cuda").long()
    ...
    def query_for_dn2(self, stage_list, queries, query_pos):
        ...
        # Handle empty query_feats case
        if len(query_feats) == 0:
            print("INFO: len(query_feats) == 0 - No valid instances for denoising in query_for_dn2")
            # No valid instances for denoising, return original queries without DN
            tgt_mask = None
            dn_args = torch.tensor([]).to("cuda").long()
            return queries, query_pos, tgt_mask, dn_args
    ...
    def query_for_dn3(self, stage_list, queries, query_pos):
        ...
        # Handle empty query_feats case
        if len(query_feats) == 0:
            print("INFO: len(query_feats) == 0 - No valid instances for denoising in query_for_dn3")
            # No valid instances for denoising, return original queries without DN
            tgt_mask = None
            dn_args = torch.tensor([]).to("cuda").long()
            return queries, query_pos, tgt_mask, dn_args
    ...
    def forward(self, stage_list,layerIds):
        ...
        if self.training:
            if len(dn_args) > 0:  # Only do postprocessing if we have DN queries
                ...
                # No DN queries, create empty DN output
                dn_out = {
                    'pred_logits': torch.empty(0, 0, self.num_classes+1).to('cuda'),
                    'pred_masks': torch.empty(0, 0, 0).to('cuda'), 
                    'aux_outputs': [],
                    'dn_args': dn_args
                }
