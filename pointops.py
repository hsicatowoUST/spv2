...
class FurthestSampling(Function):
    ...
    def forward(ctx, xyz, offset, new_offset):
        ...
        tmp = torch.zeros((n,), dtype=torch.float32, device='cuda').fill_(1e10)
...
class SectorizedFurthestSampling(Function):
    ...
    def forward(ctx, xyz, offset, new_offset, num_sectors, min_points=10000):
        ...
        idx = torch.zeros((new_sector_offset[b - 1].item()), dtype=torch.int32, device='cuda')
        # tmp = torch.cuda.FloatTensor(n).fill_(1e10)
        tmp = torch.zeros((n,), dtype=torch.float32, device='cuda').fill_(1e10)
...
class Grouping(Function):
    ...
    def forward(ctx, input, idx):
        ...
        output = torch.empty((m, nsample, c), dtype=input.dtype, device='cuda')
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = torch.zeros((n, c), dtype=grad_output.dtype, device='cuda')
...
class Subtraction(Function):
    ...
    def forward(ctx, input1, input2, idx):
        ...
        output = torch.zeros((n, nsample, c), dtype=input1.dtype, device='cuda')
    ...
    def backward(ctx, grad_output):
        ...
        grad_input1 = torch.zeros((n, c), dtype=grad_output.dtype, device='cuda')
        # grad_input2 = torch.cuda.FloatTensor(n, c).zero_()
        grad_input2 = torch.zeros((n, c), dtype=grad_output.dtype, device='cuda')
...
class Aggregation(Function):
    ...
    def forward(ctx, input, position, weight, idx):
        ...
        output = torch.zeros((n, c), dtype=input.dtype, device='cuda')
    ...
    def backward(ctx, grad_output):
        ...
        grad_position = torch.zeros((n, nsample, c), dtype=grad_output.dtype, device='cuda')

        # grad_weight = torch.cuda.FloatTensor(n, nsample, w_c).zero_()
        grad_weight = torch.zeros((n, nsample, w_c), dtype=grad_output.dtype, device='cuda')
...
def interpolation(xyz, new_xyz, feat, offset, new_offset, k=3):
    ...
    assert xyz.is_contiguous() and new_xyz.is_contiguous() and feat.is_contiguous()
    # debug
    k = min(k, xyz.shape[0])  # Ensure k does not exceed the number of points
    # debug
    ...
    new_feat = torch.zeros((new_xyz.shape[0], feat.shape[1]), dtype=feat.dtype, device='cuda')
...
class Interpolation(Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = torch.zeros((m, c), dtype=grad_output.dtype, device='cuda')
